...
<?php
	...
	if($page != 1) { 
		$paging .= '<a href="./test_board.php?page=1' . $subString . '"><li class="page_start">처음</li></a>';
	}
	//첫 섹션이 아니라면 이전 버튼을 생성
	if($currentSection != 1) { 
		$paging .= '<a href="./test_board.php?page=' . $prevPage . $subString . '"><li class="page_prev">이전</li></a>';
	}
	...
	for($i = $firstPage; $i <= $lastPage; $i++) {
		if($i == $page) {
			...
			$paging .= '<a href="./test_board.php?page=' . $i . $subString . '"><li class="page">' . $i . '</li></a>';
		}
	}
	
	//마지막 섹션이 아니라면 다음 버튼을 생성
	if($currentSection != $allSection) { 
		$paging .= '<a href="./test_board.php?page=' . $nextPage . $subString . '"><li class="page_next">다음</li></a>';
	}
	
	//마지막 페이지가 아니라면 끝 버튼을 생성
	if($page != $allPage) { 
		$paging .= '<a href="./test_board.php?page=' . $allPage . $subString . '"><li class="page_end">끝</li></a>';
	}
	$paging .= '</ul>';
...
<script>
	function writee() {
		var id='<?=$id;?>';
		if(id!="") {
			location.replace('./test_board_write.php');
		} else if(id==""){
			alert("로그인이 필요합니다.");
		}
	}
...
<div class="boardWrap">
	<article class="boardArticle">
		<div id="boardList">
			...
			<div class="searchBox">
				<form action="./test_board.php" method="get">
			...
			<table>
				...
				<tbody>
					...
					<tr>
						...
						<td class="title">&nbsp;&nbsp;&nbsp;&nbsp;<a href="./test_board_view.php?tb_num=<?=$row['tb_num']?>"><?php echo $row['tb_title']?></a></td>
